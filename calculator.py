...
def calculate():
    ...
    try:
        ...
        messagebox.showinfo('Error','Only numbers should be entered')
        calc.insert(0, 0)
    except ZeroDivisionError:
        messagebox.showinfo('Error','Can`t be divided by zero!!!')
        calc.insert(0, 0)
...
def make_calc_button(operation):
    return tk.Button(text=operation, bd=5, font=('Arial',13), fg='black', command=calculate)


def make_clear_button(operation):
    return tk.Button(text=operation, bd=5, font=('Arial',13), fg='black', command=clear)


win = tk.Tk()
win.geometry(f"280x320+100+200")
win['bg'] = '#1A1A1A'
